...
### Private / Public / Custom workspaces

- Every user owns one `Private Space`; no global administrator override is granted for another user's Private Space.
- Every active user group owns one `Public Space`; approved members can read and create projects there.
- Users can own `Custom Space` workspaces and invite active users across groups through the searchable directory.
- `WECOM_GROUP_REQUEST_WEBHOOK_URL` can point to a dedicated group-request robot. If it is empty, the backend falls back to `WECOM_FEEDBACK_WEBHOOK_URL`.
- Before the legacy `personal/shared` schema is migrated, the backend copies the SQLite database into `backend/data/backups/forecast_lab_before_workspace_model_*.sqlite`.
