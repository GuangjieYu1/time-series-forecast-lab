...
from app.db.models import AgentRunRecord, Base, ExperimentRecord, ReportRecord, UserRecord
...
def test_private_public_custom_requests_permissions_and_move(group_env: GroupTestEnv, monkeypatch):
    monkeypatch.setattr(
        "app.services.group_service.notify_group_join_request",
        lambda **_: WeComNotifyResult(status="failed", error="simulated webhook outage"),
    )
    ...
    assert any(item["experimentId"] == "exp_move" for item in user_b.get("/api/experiments").json())
    assert user_b.delete("/api/experiments/exp_move").status_code == 403
    ...
    with group_env.session_local() as db:
        ...
        agent_run = db.get(AgentRunRecord, "agent_move")
        assert experiment is not None and experiment.workspace_id == a_private_id and experiment.created_by_user_id == user_a_id
        assert report is not None and report.workspace_id == a_private_id
        assert agent_run is not None and agent_run.workspace_id == a_private_id
...
def test_legacy_workspace_migration_creates_group_public_space(tmp_path):
    ...
    with engine.connect() as connection:
        ...
        manager_role = connection.execute(text("SELECT role FROM user_group_memberships WHERE group_id = 'group_a' AND user_id = 'user_a'")).scalar_one()
        public_role = connection.execute(text("SELECT role FROM workspace_memberships WHERE workspace_id = :workspace_id AND user_id = 'user_a'"), {"workspace_id": public[0]}).scalar_one()
