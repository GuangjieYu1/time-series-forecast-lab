...
from app.schemas import (
    ...
    ReplaceWorkspaceMembersRequest,
    ...
    try:
        member_ids = list(
            dict.fromkeys(
                ...
                if user_id.strip() and user_id.strip() != current_user.id
            )
        )
        members = (
            db.scalars(select(UserRecord).where(UserRecord.id.in_(member_ids), UserRecord.is_active.is_(True))).all()
            if member_ids
            else []
        )
...
def update_workspace(
    ...
    try:
        if context.workspace.id != workspace_id:
            raise AppError("只能修改当前选中的协作空间。", 403, "WORKSPACE_OWNER_REQUIRED")
...
def delete_workspace(
    ...
    try:
        if context.workspace.id != workspace_id:
            raise AppError("只能删除当前选中的协作空间。", 403, "WORKSPACE_OWNER_REQUIRED")
...
def add_member(
    ...
    try:
        ...
        if user is None or not user.is_active:
            raise AppError("用户不存在或已停用。", 404, "USER_NOT_FOUND")
        exists = db.scalar(
            select(func.count())
            .select_from(WorkspaceMembershipRecord)
            .where(
                WorkspaceMembershipRecord.workspace_id == workspace_id,
                WorkspaceMembershipRecord.user_id == payload.userId,
            )
        )
        if exists:
...
def replace_members(
    ...
    try:
        ...
        users = (
            db.scalars(select(UserRecord).where(UserRecord.id.in_(requested_ids), UserRecord.is_active.is_(True))).all()
            if requested_ids
            else []
        )
