...
def _serialize_group(db: Session, group: UserGroupRecord) -> UserGroupSummary:
    member_count = int(
        db.scalar(select(func.count()).select_from(UserGroupMembershipRecord).where(UserGroupMembershipRecord.group_id == group.id)) or 0
    )
    manager_count = int(
        db.scalar(
            select(func.count())
...
def create_user_group(
    ...
    try:
        ...
        set_group_managers(
            db,
            group=group,
            manager_user_ids=payload.managerUserIds or [current_user.id],
        )
        db.commit()
...
def update_user_group(
    ...
    try:
        ...
        if payload.name is not None:
            name = payload.name.strip()
            duplicate = db.scalar(select(UserGroupRecord).where(UserGroupRecord.name == name, UserGroupRecord.id != group.id))
