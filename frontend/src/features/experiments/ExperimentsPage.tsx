...
export function ExperimentsPage() {
  ...
  function canManage(experiment: ExperimentListItem) {
    if (!selectedWorkspace || selectedWorkspace.kind === "example") return false;
    if (selectedWorkspace.isReadOnly) {
      return selectedWorkspace.kind === "public" && (selectedWorkspace.role === "manager" || selectedWorkspace.role === "admin");
    }
          ...
          <article key={experiment.experimentId} className="group rounded-3xl border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-xl hover:shadow-slate-200/70 dark:border-white/10 dark:bg-[#111827] dark:hover:shadow-black/30">
            ...
            <div className="mt-5 flex gap-2">
              ...
              <button
                ...
                disabled={!canManage(experiment)}
                title={!canManage(experiment) ? "只有项目创建者、空间 Owner 或组管理员可以删除；归档 Public 仅允许管理员治理。" : undefined}
